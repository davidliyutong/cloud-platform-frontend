...
build.docker.buildx: build.yarn
	docker buildx build --platform=linux/amd64,linux/arm64 -t davidliyutong/clpl-frontend:${GIT_BRANCH}-latest -t davidliyutong/clpl-frontend:${GIT_BRANCH}-${GIT_VERSION} -t davidliyutong/clpl-frontend:latest -f manifests/docker/Dockerfile .
	docker buildx build --load -t davidliyutong/clpl-frontend:latest -f manifests/docker/Dockerfile .

push.docker.buildx: build.yarn
	docker buildx build --push --platform=linux/amd64,linux/arm64 -t davidliyutong/clpl-frontend:${GIT_BRANCH}-latest -t davidliyutong/clpl-frontend:${GIT_BRANCH}-${GIT_VERSION} -t davidliyutong/clpl-frontend:latest -f manifests/docker/Dockerfile .
